...
static ssize_t dev_write(struct file *filep, const char *buffer, size_t len, loff_t *offset){
   long i, sum = 0, checksumNumber;
   ++readCounter;
   if(readCounter%2==0){
   		sprintf(checksum, "%s", buffer);
   		kstrtol(checksum, 10, &checksumNumber);
   		for(i=0; i<strlen(message); ++i)
   			sum += message[i];
   		printk(KERN_INFO "Ebb: Checksum %ld\n", checksumNumber);
   		checksumNumber = ~checksumNumber;
   		if(~(checksumNumber + sum)==0){
   			printk(KERN_INFO "Ebb: CheckSum verified successfully\n");
   		}
   		else{
   			printk(KERN_INFO "Ebb: CheckSum verification failed\n");
   		}
   }	
   else{
